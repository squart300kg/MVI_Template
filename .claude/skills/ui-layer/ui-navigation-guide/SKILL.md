---
name: ui-navigation-guide
description: Typed route, NavGraphBuilder 확장, bottom tab navigation, ViewModel navigation 요청을 추가하거나 수정할 때 사용하는 스킬.
---
...
현재 `core:router`와 typed route 기반 navigation 패턴을 유지하고, 화면 이동 요청을 ViewModel 경로로 모읍니다.
...
- Composable에서 navigation callback 경계를 정리할 때
...
private fun SampleContent(
  ...
  // Button(onClick = { onWebClick(uiState.helpUrl) }) { ... }
}
...
internal fun SampleRoute(
  ...
  SampleContent(
    ...
    onWebClick = viewModel::navigateWeb,
  )
}
```
