...
- 전역 progress와 에러 메시지 다이얼로그는 feature Composable이 직접 그리지 않고 `MainActivity`의 `GlobalUiBus` 구독 경로를 사용합니다.
- Screen/Content Composable은 `NavHostController`를 직접 받지 않고 ViewModel event/callback만 호출합니다.
- 화면 작업 후 Preview를 추가합니다.

## 텍스트 렌더링 기준

- `String` 상태는 `Text(value)`로 직접 표시합니다.
- `UiText` 상태는 Composable에서만 `asString()`으로 resolve합니다.
...
6. PreviewParameterProvider 또는 간단 Preview로 주요 상태를 확인합니다.
