...
1. merge 요청이면 최신 `origin/<base>` 위로 다시 rebase하고, base 변경이 있으면 충돌과 의미 충돌을 모두 resolve한 뒤 검증을 재실행합니다.
2. 여러 PR merge 요청이면 열린 PR 목록과 우선순위를 먼저 사용자에게 보고합니다.
3. 검증 통과 후 `gh pr merge --squash`로 병합합니다.
4. 병합된 branch와 이 스킬이 만든 clean worktree를 정리합니다.
