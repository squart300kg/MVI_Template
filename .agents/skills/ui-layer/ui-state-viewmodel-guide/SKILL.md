...
1. suspend 작업은 `launchWithCatching`으로 감싸 전역 loading/error 처리를 위임합니다.
2. navigation은 typed route를 사용해 `navigateTo(...)`, `navigateBack()`, `navigateWeb(...)`로 호출합니다.

### 모델과 검증

1. 화면 표시용 model이 필요하면 재사용 범위에 따라 feature-local 또는 `core:ui` 위치를 정합니다.
2. DTO/domain model에서 화면 model로 변환하는 top-level extension mapper를 추가합니다.
3. 테스트에서 fake repository/use case로 state와 effect를 검증합니다.
