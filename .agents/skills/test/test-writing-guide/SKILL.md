...
기능 요구사항과 회귀 조건이 테스트 이름과 assertion에 직접 드러나게 합니다.
...
- 테스트 함수명은 한국어 백틱 문장으로 작성합니다.
- 테스트 시나리오, fixture 이름, assertion 메시지는 요구사항을 한국어로 드러내되 production identifier는 기존 영어 이름을 유지합니다.
...
```kotlin
@Test
fun `목록 조회에 성공하면 화면 상태에 아이템을 반영한다`() = runTest {
  // given
  // when
  // then
}
