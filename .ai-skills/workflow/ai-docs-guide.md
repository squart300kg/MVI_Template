...
3. 스킬 책임을 한 문장으로 정의하고, 사용자 요청이 어떤 표현일 때 트리거되어야 하는지 `description`에 적습니다.
4. 기본 6개 섹션으로 본문을 먼저 작성하고, 추가 목차가 필요하면 기존 기본 섹션 아래 `###`로 넣습니다.
5. 절차와 판단 기준을 분리합니다. 절차는 순서이고, 판단 기준은 분기 조건입니다.
6. 출력 섹션에는 최종 답변, 파일, 템플릿, 명령 결과처럼 사용자가 받을 산출물을 적습니다.
7. 점검 섹션에는 끝내기 전에 확인할 실패 방지 기준을 적습니다.
8. `./scripts/sync-harness-docs.sh copy`를 실행합니다.
9. `./scripts/verify-harness-consistency.sh`로 mirror 정합성을 확인합니다.
10. Gradle task가 필요한 변경이면 `./gradlew verifyHarnessConsistency`도 실행합니다.
