...
- `BaseCenterDialog`는 `MainActivity`에서 `globalUiBus.failureDialog`를 구독해 표시하는 전역 경로를 우선합니다.
...
2. 에러 메시지라면 feature dialog를 만들지 않고 ViewModel의 `launchWithCatching` 또는 `globalUiBus.showFailureDialog(...)`를 사용합니다.
